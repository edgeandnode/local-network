...
1. Local network running with the eligibility-oracle override:
   ```bash
   docker compose -f docker-compose.yaml \
     -f overrides/eligibility-oracle/docker-compose.yaml up -d --build
   ```
...
4. REO node running and connected:
   ```bash
   docker compose -f docker-compose.yaml \
     -f overrides/eligibility-oracle/docker-compose.yaml logs --tail 20 eligibility-oracle-node
   ```
...
docker compose -f docker-compose.yaml \
  ...
  logs -f eligibility-oracle-node
```
