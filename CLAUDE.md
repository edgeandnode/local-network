...
Most services run from prebuilt images pinned by `${SERVICE_VERSION}` variables in the `config/*.env` fragments. The `graph-contracts` image consumes a tagged contracts image (`CONTRACTS_VERSION`); `subgraph-deploy` clones `graph-network-subgraph` and `block-oracle` at the commit hashes pinned in those fragments (`NETWORK_SUBGRAPH_COMMIT`, `BLOCK_ORACLE_COMMIT`) and consumes a tagged indexing-payments-subgraph image (`INDEXING_PAYMENTS_SUBGRAPH_VERSION`). To run an upstream service from unmerged source, build a `:local`-tagged image in that repo and override its version pin in `.env.local` (see the README's "Iterating on upstream source").

## Key Config

- `.env` is what docker-compose, host scripts, and containers read (the latter via volume mount at `/opt/config/.env`), but it is generated by `scripts/resolve-recipe.sh` from a recipe (`recipes/*.json`) plus `config/*.env` fragments and is gitignored. Edit the fragments or `.env.local`, not `.env` directly.
