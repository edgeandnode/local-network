...
cat subgraph.yaml
npx graph create block-oracle --node="http://host.docker.internal:${GRAPH_NODE_ADMIN}"
npx graph deploy block-oracle --node="http://host.docker.internal:${GRAPH_NODE_ADMIN}" \
  --ipfs="http://host.docker.internal:${IPFS_RPC}" --version-label 'v0.0.1' \
  | tee deploy.txt
deployment_id="$(grep "Build completed: " deploy.txt | awk '{print $3}' | sed -e 's/\x1b\[[0-9;]*m//g')"
echo "${deployment_id}"
curl "http://host.docker.internal:${GRAPH_NODE_ADMIN}" \
...
subgraph_url = "http://host.docker.internal:${GRAPH_NODE_GRAPHQL}/subgraphs/name/block-oracle"
bearer_token = "TODO"
log_level = "trace"

[protocol_chain]
name = "eip155:1337"
jrpc = "http://host.docker.internal:8545"
polling_interval_in_seconds = 20

[indexed_chains]
"eip155:1337" = "http://host.docker.internal:8545"
EOF
cat config.toml
