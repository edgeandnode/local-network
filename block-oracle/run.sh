...
epoch_manager="$(curl "http://controller:${CONTROLLER}/graph_contracts" | jq -r '."1337".EpochManager.address')"

sed -i "s+http://localhost:8545+http://chain:${CHAIN_RPC}+g" hardhat.config.ts
yarn
...
curl "http://controller:${CONTROLLER}/block_oracle_subgraph" -d "${deployment_id}"
...
export SUBGRAPH_URL="http://graph-node:${GRAPH_NODE_GRAPHQL}/subgraphs/name/block-oracle"
export PROTOCOL_CHAIN_JRPC_URL="http://chain:${CHAIN_RPC}"
