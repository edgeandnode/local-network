...
indexer_staked="$(cast call "--rpc-url=http://host.docker.internal:${CHAIN_RPC}" \
...
if [ "${indexer_staked}" = "false" ]; then
  ...
  cast send "--rpc-url=http://host.docker.internal:${CHAIN_RPC}" --confirmations=0 "--mnemonic=${MNEMONIC}" \
  ...
  cast send "--rpc-url=http://host.docker.internal:${CHAIN_RPC}" --confirmations=0 "--private-key=${RECEIVER_SECRET}" \
...
export INDEXER_AGENT_EPOCH_SUBGRAPH_ENDPOINT="http://host.docker.internal:${GRAPH_NODE_GRAPHQL}/subgraphs/name/block-oracle"
export INDEXER_AGENT_GATEWAY_ENDPOINT="http://host.docker.internal:${GATEWAY}"
export INDEXER_AGENT_GRAPH_NODE_QUERY_ENDPOINT="http://host.docker.internal:${GRAPH_NODE_GRAPHQL}"
export INDEXER_AGENT_GRAPH_NODE_ADMIN_ENDPOINT="http://host.docker.internal:${GRAPH_NODE_ADMIN}"
export INDEXER_AGENT_GRAPH_NODE_STATUS_ENDPOINT="http://host.docker.internal:${GRAPH_NODE_STATUS}/graphql"
...
export INDEXER_AGENT_NETWORK_SUBGRAPH_ENDPOINT="http://host.docker.internal:${GRAPH_NODE_GRAPHQL}/subgraphs/name/graph-network"
export INDEXER_AGENT_NETWORK_PROVIDER="http://host.docker.internal:${CHAIN_RPC}"
...
export INDEXER_AGENT_TAP_SUBGRAPH_ENDPOINT="http://host.docker.internal:${GRAPH_NODE_GRAPHQL}/subgraphs/semiotic/tap"
mkdir -p ./config/
