...
"$SCRIPT_DIR/mine-block.sh" 5 > /dev/null 2>&1
...
if [ "$eligible_before" = "false" ] && [ "$eligible_after" = "true" ]; then
  ...
  echo "  Check REO logs: docker compose -f docker-compose.yaml -f overrides/eligibility-oracle/docker-compose.yaml logs --tail 50 eligibility-oracle-node"
