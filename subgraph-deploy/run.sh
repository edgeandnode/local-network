...
block_oracle_deployment="$(curl "http://host.docker.internal:${GRAPH_NODE_GRAPHQL}/subgraphs/name/block-oracle" \
  -H 'content-type: application/json' \
  -d '{"query": "{ _meta { deployment } }" }' \
  | jq -r '.data._meta.deployment')"
tap_deployment="$(curl "http://host.docker.internal:${GRAPH_NODE_GRAPHQL}/subgraphs/name/semiotic/tap" \
  -H 'content-type: application/json' \
  -d '{"query": "{ _meta { deployment } }" }' \
  | jq -r '.data._meta.deployment')"

# force index block oracle subgraph & network subgraph
graph-indexer indexer connect "http://host.docker.internal:${INDEXER_MANAGEMENT}"
...
deployment_hex="$(curl -X POST "http://host.docker.internal:${IPFS_RPC}/api/v0/cid/format?arg=${block_oracle_deployment}&b=base16" \
  | jq -r '.Formatted')"
...
while ! graph-indexer indexer --network=hardhat actions get | grep 'allocate'; do
  cast rpc --rpc-url="http://host.docker.internal:${CHAIN_RPC}" evm_mine
  sleep 2
done

# wait for an active allocation
while ! curl "http://host.docker.internal:${GRAPH_NODE_GRAPHQL}/subgraphs/name/graph-network" \
  -H 'content-type: application/json' \
